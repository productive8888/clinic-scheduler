import type { Employee, NPTORequest } from "@prisma/client";
import { CalendarX, Check, RotateCcw, ShieldAlert, Undo2, X } from "lucide-react";
import {
  approveNptoRequestAction,
  cancelNptoAsAdminAction,
  overrideNptoRequestAction,
  rejectNptoRequestAction,
  returnNptoToPendingAction,
  reverseNptoApprovalAction,
} from "@/app/(app)/admin/pto/actions";
import { cancelMyNptoRequestAction } from "@/app/(app)/employee/actions";
import { ShortNoticeBadge } from "@/components/ui/short-notice-badge";
import { formatDisplayDate } from "@/lib/utils/date";
import { formatMinuteOfDay } from "@/lib/utils/time";

type NPTORequestRecord = NPTORequest & {
  employee: Employee;
  reviewedBy: Employee | null;
};

type NPTORequestListProps = {
  requests: NPTORequestRecord[];
  mode: "employee" | "manager";
};

const statusStyles = {
  PENDING: "bg-amber-50 text-amber-800",
  APPROVED: "bg-emerald-50 text-emerald-800",
  REJECTED: "bg-rose-50 text-rose-800",
  CANCELLED: "bg-slate-100 text-slate-600",
  REVERSED: "bg-sky-50 text-sky-800",
  OVERRIDDEN: "bg-violet-50 text-violet-800",
};

export function NPTORequestList({ requests, mode }: NPTORequestListProps) {
  if (requests.length === 0) {
    return (
      <div className="rounded-md border border-dashed border-slate-300 bg-white p-8 text-center text-sm text-slate-500">
        No NPTO requests yet.
      </div>
    );
  }

  return (
    <div className="grid gap-3">
      {requests.map((request) => (
        <article
          key={request.id}
          className="rounded-md border border-slate-200 bg-white p-4 shadow-sm"
        >
          <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-start">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="font-semibold text-slate-950">
                  {mode === "manager" ? request.employee.fullName : "NPTO"}
                </h3>
                <span
                  className={`rounded-md px-2 py-1 text-xs font-semibold ${statusStyles[request.status]}`}
                >
                  {request.status}
                </span>
                <span className="rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600">
                  {request.requestedHours.toString()}h requested
                </span>
                {Number(request.unpaidHours) > 0 ? (
                  <span className="rounded-md bg-slate-950 px-2 py-1 text-xs font-semibold text-white">
                    {request.unpaidHours.toString()}h unpaid
                  </span>
                ) : null}
                {request.shortNotice ? <ShortNoticeBadge /> : null}
              </div>
              <p className="mt-2 flex flex-wrap items-center gap-2 text-sm text-slate-600">
                <CalendarX size={16} aria-hidden="true" />
                {formatDisplayDate(request.startDate)} to{" "}
                {formatDisplayDate(request.endDate)}
                {formatTimeRange(request)}
              </p>
              {request.reason ? (
                <p className="mt-2 text-sm text-slate-500">{request.reason}</p>
              ) : null}
              {request.denialReason ? (
                <p className="mt-2 rounded-md bg-rose-50 p-2 text-sm text-rose-800">
                  <span className="font-semibold">Denial reason: </span>
                  {request.denialReason}
                </p>
              ) : null}
              {request.managerNote ? (
                <p className="mt-2 rounded-md bg-slate-50 p-2 text-sm text-slate-600">
                  <span className="font-semibold text-slate-800">
                    Manager note:{" "}
                  </span>
                  {request.managerNote}
                </p>
              ) : null}
              {request.status === "PENDING" ? (
                <p className="mt-2 text-xs text-amber-700">
                  Awaiting manager review. NPTO does not reduce PTO balance.
                </p>
              ) : null}
              {request.reviewedBy ? (
                <p className="mt-2 text-xs text-slate-500">
                  Reviewed by {request.reviewedBy.fullName}
                </p>
              ) : null}
            </div>

            {mode === "manager" ? (
              <ManagerNptoActions request={request} />
            ) : null}

            {mode === "employee" && request.status === "PENDING" ? (
              <form action={cancelMyNptoRequestAction.bind(null, request.id)}>
                <button className="inline-flex h-10 items-center justify-center gap-2 rounded-md border border-slate-300 px-3 text-sm font-semibold text-slate-700 hover:bg-slate-100">
                  <X size={16} aria-hidden="true" />
                  Cancel request
                </button>
              </form>
            ) : null}
          </div>
        </article>
      ))}
    </div>
  );
}

function ManagerNptoActions({ request }: { request: NPTORequestRecord }) {
  if (request.status === "PENDING") {
    return (
      <div className="grid gap-2 sm:grid-cols-2 lg:min-w-96 xl:grid-cols-4">
        <NPTOActionForm
          action={approveNptoRequestAction.bind(null, request.id)}
          notePlaceholder="Approval note"
          buttonLabel="Approve"
          tone="approve"
          icon={<Check size={16} aria-hidden="true" />}
        />
        <NPTOActionForm
          action={rejectNptoRequestAction.bind(null, request.id)}
          notePlaceholder="Rejection reason"
          buttonLabel="Reject"
          tone="reject"
          icon={<X size={16} aria-hidden="true" />}
        />
        <NPTOActionForm
          action={overrideNptoRequestAction.bind(null, request.id)}
          notePlaceholder="Override reason"
          buttonLabel="Override"
          tone="neutral"
          icon={<ShieldAlert size={16} aria-hidden="true" />}
        />
        <NPTOActionForm
          action={cancelNptoAsAdminAction.bind(null, request.id)}
          notePlaceholder="Cancel note"
          buttonLabel="Cancel"
          tone="neutral"
          icon={<X size={16} aria-hidden="true" />}
        />
      </div>
    );
  }

  if (request.status === "APPROVED" || request.status === "OVERRIDDEN") {
    return (
      <div className="lg:min-w-72">
        <NPTOActionForm
          action={reverseNptoApprovalAction.bind(null, request.id)}
          notePlaceholder="Reversal note"
          buttonLabel="Reverse approval"
          tone="neutral"
          icon={<RotateCcw size={16} aria-hidden="true" />}
        />
      </div>
    );
  }

  if (
    request.status === "REJECTED" ||
    request.status === "REVERSED" ||
    request.status === "CANCELLED"
  ) {
    return (
      <div className="grid gap-2 sm:grid-cols-2 lg:min-w-96">
        <NPTOActionForm
          action={returnNptoToPendingAction.bind(null, request.id)}
          notePlaceholder="Pending note"
          buttonLabel="Return pending"
          tone="neutral"
          icon={<Undo2 size={16} aria-hidden="true" />}
        />
        <NPTOActionForm
          action={overrideNptoRequestAction.bind(null, request.id)}
          notePlaceholder="Override reason"
          buttonLabel="Override"
          tone="neutral"
          icon={<ShieldAlert size={16} aria-hidden="true" />}
        />
      </div>
    );
  }

  return null;
}

function NPTOActionForm({
  action,
  notePlaceholder,
  buttonLabel,
  tone,
  icon,
}: {
  action: (formData: FormData) => void | Promise<void>;
  notePlaceholder: string;
  buttonLabel: string;
  tone: "approve" | "reject" | "neutral";
  icon: React.ReactNode;
}) {
  const buttonClassName =
    tone === "approve"
      ? "inline-flex h-10 items-center justify-center gap-2 rounded-md bg-emerald-700 px-3 text-sm font-semibold text-white hover:bg-emerald-800"
      : tone === "reject"
        ? "inline-flex h-10 items-center justify-center gap-2 rounded-md border border-rose-200 px-3 text-sm font-semibold text-rose-700 hover:bg-rose-50"
        : "inline-flex h-10 items-center justify-center gap-2 rounded-md border border-slate-300 px-3 text-sm font-semibold text-slate-700 hover:bg-slate-100";

  return (
    <form action={action} className="grid gap-2">
      <input
        name="managerNote"
        placeholder={notePlaceholder}
        className="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-emerald-700"
      />
      <button className={buttonClassName}>
        {icon}
        {buttonLabel}
      </button>
    </form>
  );
}

function formatTimeRange(request: NPTORequest) {
  const start = formatMinuteOfDay(request.startMinute);
  const end = formatMinuteOfDay(request.endMinute);

  return start && end ? `, ${start}-${end}` : "";
}
